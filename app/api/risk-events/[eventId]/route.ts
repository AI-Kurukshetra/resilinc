...
import { getRiskEventById, RiskEventServiceError } from "@/lib/risk-events/ingestion";

interface RiskEventRouteContext {
  params: Promise<{ eventId: string }>;
}

export async function GET(_request: Request, { params }: RiskEventRouteContext) {
  ...
  try {
    const event = await getRiskEventById(supabase, organizationId, paramParsed.data.eventId);
    ...
    return apiError(
      { code: "RISK_EVENT_FETCH_FAILED", message: "Unexpected error fetching risk event." },
      500,
    );
  }
}
